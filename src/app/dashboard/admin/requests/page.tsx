...
export default function AdminRequestsPage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-text-primary">Dispatch Requests</h1>
                <p className="text-sm text-text-secondary mt-1">Manage all material dispatch requests</p>
            </div>

            <DataCard title="Pending Requests" subtitle={`${pending.length} awaiting action`}>
                {loading ? <div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="skeleton h-12" />)}</div> : pending.length === 0 ? (
                    <EmptyState icon={ClipboardList} title="All Clear" description="No pending dispatch requests." />
                ) : (
                        ...
                        {pending.map(r => (
                            <div key={r.id} className="flex items-center justify-between p-4 rounded-xl bg-surface border border-border-light card-hover">
                                <div>
                                    ...
                                    <p className="text-xs text-text-secondary">Qty: {r.quantity} · By: {r.requestedBy} · Op: {r.operatorId}</p>
            ...
            <DataCard title="Processed Requests" subtitle={`${processed.length} completed`}>
                {processed.length === 0 ? (
                    <EmptyState icon={ClipboardList} title="No History" description="No processed requests yet." />
                ) : (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>{processed.map(r => (
                                <tr key={r.id} className="border-b border-border-light last:border-0 hover:bg-surface">
                                    ...
                                    <td className="py-3 px-4 font-mono text-xs text-secondary">{r.operatorId}</td>
